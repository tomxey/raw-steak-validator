...
describe('computeEpochYields', () => {
    ...
    it('handles large withdrawal spike (Figment scenario)', () => {
        ...
        // Epoch 105: rate jumps 10x normal yield
    ...
    it('skips non-consecutive epochs (gap > 1)', () => {
        const entries: EpochRateEntry[] = [
            { epoch: 10, iotaAmount: 1e9, poolTokenAmount: 1e9, rate: 1.0 },
            { epoch: 13, iotaAmount: 1.0003e9, poolTokenAmount: 1e9, rate: 1.0003 },
        ];
        const yields = computeEpochYields(entries);
        expect(yields).toHaveLength(0); // gap=3, skipped
    });

    it('skips non-consecutive but keeps consecutive pairs', () => {
        const entries: EpochRateEntry[] = [
            { epoch: 10, iotaAmount: 1e9, poolTokenAmount: 1e9, rate: 1.0 },
            { epoch: 13, iotaAmount: 1.0003e9, poolTokenAmount: 1e9, rate: 1.0003 }, // gap=3, skip
            { epoch: 14, iotaAmount: 1.0004e9, poolTokenAmount: 1e9, rate: 1.0004 }, // gap=1, keep
        ];
        const yields = computeEpochYields(entries);
        expect(yields).toHaveLength(1);
        expect(yields[0].epoch).toBe(14);
    });
    ...
    it('skips entries with zero or negative epoch gap', () => {
        ...
        const yields = computeEpochYields(entries);
        // First pair skipped (gap=0), second pair: epoch 10→11, gap=1
        expect(yields).toHaveLength(1);
        expect(yields[0].epoch).toBe(11);
    });
...
describe('computeAverageApy', () => {
    ...
    it('returns 0 for zero window', () => {
        const yields: EpochYieldEntry[] = [
            { epoch: 11, perEpochYield: 0.0001, annualizedApy: 3.65 },
        ];
    ...
    it('one spike among steady epochs has limited effect on average', () => {
        ...
        const yields: EpochYieldEntry[] = Array.from({ length: 10 }, (_, i) => ({
            epoch: 101 + i,
            perEpochYield: i === 9 ? spike : steady,
    ...
    it('uses all available data when window exceeds data length', () => {
        const perEpochYield = 0.0001;
        const yields: EpochYieldEntry[] = [
            { epoch: 101, perEpochYield, annualizedApy: perEpochYield * 365 * 100 },
            { epoch: 102, perEpochYield, annualizedApy: perEpochYield * 365 * 100 },
        ];
    ...
    it('window of 1 equals latest', () => {
        const yields: EpochYieldEntry[] = [
            { epoch: 101, perEpochYield: 0.0001, annualizedApy: 3.65 },
            { epoch: 102, perEpochYield: 0.0002, annualizedApy: 7.30 },
        ];
    ...
    it('simple average of last N entries', () => {
        const yields: EpochYieldEntry[] = [
            { epoch: 101, perEpochYield: 0.0001, annualizedApy: 3.65 },
            { epoch: 102, perEpochYield: 0.0003, annualizedApy: 10.95 },
        ];

        // Window of 2: simple avg = (0.0001 + 0.0003) / 2 = 0.0002
...
describe('computeValidatorApyHistory', () => {
    ...
    it('Figment scenario: 29 steady epochs + 1 massive spike', () => {
        ...
        // Add epoch 130 with a 50x spike
        const lastRate = entries[entries.length - 1].rate;
    ...
    it('skips epochs with gaps and only uses consecutive pairs', () => {
        const entries: EpochRateEntry[] = [
            { epoch: 100, iotaAmount: 1e9, poolTokenAmount: 1e9, rate: 1.0 },
            { epoch: 105, iotaAmount: 1.0005e9, poolTokenAmount: 1e9, rate: 1.0005 }, // gap=5, skip
            { epoch: 106, iotaAmount: 1.0006e9, poolTokenAmount: 1e9, rate: 1.0006 }, // gap=1, keep
        ];
        const result = computeValidatorApyHistory(entries);
        expect(result.epochYields).toHaveLength(1);
        // Yield from epoch 105→106
        expect(result.epochYields[0].epoch).toBe(106);
        expect(result.epochYields[0].perEpochYield).toBeCloseTo(
            (1.0006 - 1.0005) / 1.0005, 6,
        );
    });
...
describe('estimatePostRestakeYield', () => {
    ...
    it('accounts for pending operations in effective commission', () => {
        const target: ValidatorStakeInfo = {
            poolStake: 50_000_000,
            pendingStake: 10_000_000,
            pendingWithdraw: 0,
            commission: 3,
            perEpochYield: 0.00012,
        };
...
describe('financial scenarios', () => {
    it('break-even with averaged vs single-epoch APY gives different advice', () => {
        const yieldA = 0.0001;
        const yieldB_latest = 20 / (365 * 100);
        const yieldB_avg7 = 5 / (365 * 100);
        ...
        expect(breakEvenLatest.breakEvenEpochs).toBeLessThan(5);
